...
test_that("Test of the digest adding function",{
  ...
  add_digests(ae_test,algorithm="md5",verbose=FALSE)
  add_digests(ae_test,verbose=FALSE)
  add_digests(ae_test,algorithm="sha512",verbose=FALSE)

  md <- get_metadata(ae_test, verbose=FALSE)
...
test_that("Metadata is collected correctly by get_metadata",{
  ...
  res <- get_metadata(ae_test, clearCache=TRUE, verbose=FALSE)
  ...
  outputShouldBe <- tibble::as_tibble(openxlsx::read.xlsx(file.path("..","metadata_extras","expected_meta.xlsx"),sheet="bundles"))
...
test_that("Import of metadata from an Excel file produces an exected result",{
  ...
  dirs <- gsub("0000_ses","0001_ses",dirs)
  for(currDir in dirs){
    dir.create(currDir,showWarnings = TRUE,recursive = TRUE)
  }
  ...
  make_dummy_metafiles(ae_test)
  dummyRes <- get_metadata(ae_test, clearCache=TRUE, verbose=FALSE)
  ...
  import_metadata(ae_test,file.path("..","metadata_extras","expected_meta.xlsx"),verbose=FALSE)
  importRes <- get_metadata(ae_test, verbose=FALSE)
  importRes <- importRes %>%
