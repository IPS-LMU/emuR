...
Whenever we use a name `_text` in the following we imply a varying prefix name (or base name) before the `_` while the `text`
is an obligatory string, e.g. `_bndl` implies file names such as `rec1_bndl`, `rec2_bndl` of the type `bundle folder`. The extension `.json` denotes a text file in JSON format.
...
- the root folder of the emuDB must contain exactly one `_DBconfig.json` file. It is recommended that the prefix of the `_DBconfig.json` file matches the value of the field `name` within the `_DBconfig.json` file (which specifies the official name of the emuDB), and that the root folder of the emuDB has the same prefix name as well (possibly extended by a `_emuDB`).
- all session folders must be named using the suffix `_ses`. Their prefixes can be chosen by the database maintainer.
- all bundle folders must be named using suffix `_bndl`. Their prefixes can be chosen by the database maintainer; prefixes must be unique within a session but not across the emuDB.
...
- the (single) annotation file within a bundle `basename_bndl` must be named `basename_annot.json`.

Files that do not follow this naming convention will simply be ignored by the database interaction functions of the `emuR` package (for instance multiple channels stored parallel in individual files)
...
*NOTE: It is also possible to define label groups for the entire DB. For more information on this see the R documentation for the `add/list/remove_labelGroups` functions (not yet accessible in the `query()` function!).*
...
Finally, let us assume that we have finished working on the 'fromScratchDB' emuDB and purge it from our current R session:

```{r}
# list currently loaded emuDBs
list_emuDBs()
# purge the fromScratchDB
purge_emuDB(dbName, interactive = FALSE)
# list currently loaded emuDBs
list_emuDBs()
```
*NOTE: All changes to the created emuDB we applied in the above tutorial as well as all (saved) changes we apply in the EMU-webApp will be written to the file system of the emuDB. It is therefore safe to quit the R session without 'saving' the emuDB.*
...
# load database
dbName = load_emuDB(file.path(tempdir(), "myTGcolDB"), verbose = F)
```
...
![Alt text](pics/autobuild_linkFromTimesGraph.svg)
...
```{r}
# add linkDefinition
add_linkDefinition(dbName, type = "ONE_TO_MANY",
                   superlevelName = "Syllable",
                   sublevelName = "Phoneme")

# list
list_linkDefinitions(dbName)

# envoke autobuild function
autobuild_linkFromTimes(dbName, 
                        superlevelName = "Syllable",
                        sublevelName = "Phoneme",
...
```{r}
# add linkDefinition
add_linkDefinition(dbName, type = "MANY_TO_MANY",
...
# and purge all loaded emuDBs
purge_all_emuDBs(interactive = FALSE)
```
...
- `"mediafileExtension"` the main mediafileExtension (*currently only uncompressed mono 16-bit `.wav` files are supported in every component of the EMU system. This is also the recommended audio format for the EMU_SDMS.*)
...
- `"EMUwebAppConfig"` specifies the configuration options intended for the EMU-webApp, i.e. how the database is to be displayed. This field can contain all the configurations options that are specified in the EMU-webApp's configuration schema (*https://github.com/IPS-LMU/EMU-webApp/blob/master/dist/schemaFiles/emuwebappConfigSchema.json*). The `"EMUwebAppConfig"` contains the following fields:
    ...
    - `"keyMappings"` keyboard shortcut definitions. For the sake of brevity not every keycode is shown (see schema for extensive list)
        - `"toggleSideBarLeft"` integer value that represents the keycode that toggles the left side bar (== bundleList side bar)
        - `"toggleSideBarRight"` integer value that represents the keycode that toggles the right side bar (== perspective side bar)
        - ...
    ...
    - `"labelCanvasConfig"` configuration options for the label canvases
        - `"addTimeMode"` mode to add / subtrakt time to boundaries 
        - `"addTimeValue"`: amount of samples added / subtrakted to boundaries
    ...
    - `"demoDBs"` array of strings specifying which demoDBs to display in the `open demo` dropdown menu. Currently available demo databases are `["ae", "ema", "epgdorsal"]`
